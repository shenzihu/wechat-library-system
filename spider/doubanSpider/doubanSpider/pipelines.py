...
class DoubanSpiderPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        cursor = db.cursor()
        # cursor.execute("select isbn from isbn where status = 0")
        ...
        # sql = "update book set book_name = %s, book_author = %s, book_desc = %s where  book_number =%s"
        # cursor.execute(sql, (item["book_name"], item["book_author"], item["book_desc"], item["originalISBN"]))
        sql2 = "update isbn set status = 1, get_time =now(),source = '豆瓣补充' where isbn = '%s'" % (item["originalISBN"].strip())
        print(sql2)
        cursor.execute(sql2)
        db.commit()
