...
public class BookHandler {
    ...
    public String listShow(Map<String, Object> requestMap, @RequestParam("page") Integer page,@RequestParam("status") Integer status) {
        ...
        if(status == 4){
            ...
            books = bookService.selectBookByStatus(status);
        }
    ...
    public String addBookExcute(@RequestParam("bookName") String bookName, @RequestParam("bookAuthor") String bookAuthor,
                                ...
                                @RequestParam("bookNumber") String bookNumber, @RequestParam("bookBigImage") String bookBigImage, @RequestParam("bookSmallImage") String bookSmallImage) {
        bookService.addBook(bookNumber, bookName, bookAuthor, bookPress, bookBigImage, bookSmallImage,
                bookClassifyOne, bookClassifyTwo, bookDesc);
        String view = "redirect:/admin-book-list-show?page=1&status="+4;
        return view;
    ...
    public String editBookExecute(@RequestParam("bookName") String bookName, @RequestParam("bookAuthor") String bookAuthor,
                                  ...
                                  @RequestParam("bookClassifyTwo") String bookClassifyTwo, @RequestParam("bookDesc") String bookDesc,
                                  @RequestParam("bookNumber") String bookNumber,@RequestParam("bookBigImage") String bookBigImage, @RequestParam("bookSmallImage") String bookSmallImage,
                                  @RequestParam("status") Integer status,@RequestParam("bookId") Integer id) {

        bookService.updateBook(id, bookNumber, bookName, bookAuthor, bookPress,
                bookBigImage, bookSmallImage, bookClassifyOne, bookClassifyTwo, bookDesc,status);

        return "redirect:/admin-book-list-show?page=1&status="+4;
    }
    ...
    @RequestMapping(value = "admin-book-check-execute/{id}/{status}", method = RequestMethod.GET)
    public String checkOneBook(@PathVariable("id") Integer id, @PathVariable("status") Integer status) {
        bookService.checkOneBook(id,status);
        return "redirect:/admin-book-list-show?page=1&status="+2;
    }

    @RequestMapping(value = "admin-book-check-all-pass", method = RequestMethod.POST)
    @ResponseBody
    public String checkAllPass(@RequestBody List<Integer> ids) {
        if(ids != null){
            ids.forEach(x->bookService.checkOneBook(x,1));
        }
        return "成功";
    }
}
